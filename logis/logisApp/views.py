...
from .forms import MunicipioForm, UrnaForm, MaterialEleitoralForm, ZonaEleitoralForm
from django.db.models import Sum, Count
from .models import ZonaEleitoral, Urna, ZE_solicita_urna
...
def get_qtd_urnas(zona):
    return ZE_solicita_urna.objects.filter(zona_eleitoral=zona).aggregate(total=Sum('quantidade'))['total'] or 0
...
def add_primary_model(zona, modelo, quantidade):
    ...
    urna.qtd += quantidade
    urna.save()
    ZE_solicita_urna.objects.create(zona_eleitoral=zona, urna=urna, quantidade=quantidade)
#Não está sendo utilizado atualmente, será ao alterar a View distribuir_tudo#
...
def index(request):
    ...
    solicitacoes_urnas = ZE_solicita_urna.objects.all()
    ...
    try:
        zona_estoque = ZonaEleitoral.objects.get(id=53)
...
def distribuir_tudo(request):
    zona_estoque_id = request.GET.get('zona_estoque_id')  # Assumindo que o ID da zona de estoque é passado como parâmetro na URL
    ...
    zonas = ZonaEleitoral.objects.exclude(id=zona_estoque_id).order_by('id')  # Excluir a zona de estoque da distribuição
    urnas_estoque = Urna.objects.filter(zona_eleitoral=zona_estoque).order_by('-modelo')  # Urnas do estoque
    messages = []
